...
export function CartDrawer() {
  ...
  return (
    <div role="complementary" aria-label="Sepetim"
      ...
      <div className="flex items-center justify-between border-b border-brown/10 px-5 py-4">
        <p className="font-display text-lg font-extrabold text-brown-darker">
          Sepetim <span className="text-sm font-semibold text-brown-dark/50">({lines.reduce((n, l) => n + l.quantity, 0)} ürün)</span>
        </p>
        <button type="button" onClick={closeCartDrawer} aria-label="Kapat"
        ...
        <ul className="space-y-3">
          {lines.map((line) => (
            <li key={line.slug} className="flex gap-3 rounded-2xl border border-brown/10 bg-white/60 p-2.5">
              <Link href={`/urun/${line.slug}`} className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-brown/5">
      ...
      <div className="border-t border-brown/10 px-5 py-4">
        ...
        <Link href="/odeme" className="mt-3 block rounded-full bg-brown-darker py-3 text-center text-xs font-bold text-cream transition hover:bg-green">
