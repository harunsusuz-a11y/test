  ...
  useEffect(() => {
    Promise.all([
      ...
      if (items?.length) {
        ...
        const marka = items.filter((i) => ["/hakkimizda", "/sss", "/iletisim"].includes(i.url));
        const kurumsal = items.filter((i) => ["/gizlilik", "/kvkk", "/mesafeli-satis", "/iade-teslimat", "/cerez-politikasi"].includes(i.url));
  ...
  return (
    <footer className="relative overflow-hidden bg-[#1a0d0b] text-cream/80">
        ...
        <div className="mx-auto max-w-6xl px-5">
          <ul className="grid grid-cols-2 divide-x divide-y divide-cream/[0.06] sm:grid-cols-4 sm:divide-y-0">
            {TRUST_ITEMS.map((item) => (
              <li key={item.label} className="flex flex-col items-center gap-1 px-4 py-6 text-center">
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-16">
        {/* Üst bölüm: logo + nav kolonları */}
        <div className="flex flex-col gap-12 md:flex-row md:gap-8">
          {/* Sol: marka */}
          <div className="flex flex-col gap-5 md:max-w-[260px] md:flex-shrink-0">
            <Link href="/" aria-label="Venti-Ate Ana Sayfa">
              <span className="inline-block font-display text-2xl font-bold leading-none tracking-tight text-peach">
                venti&#8209;ate
              </span>
            </Link>
            <p className="text-sm leading-relaxed text-cream/55">
              {tagline} — Giresun fındığından üretilen, gerçek içerikli, yüksek proteinli atıştırmalıklar.
            </p>
            <div className="flex items-center gap-3">
              <a
                href="https://instagram.com/ventiate.co"
                target="_blank"
          ...
          {/* Sağ: nav kolonları */}
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 md:flex md:flex-1 md:justify-end md:gap-16">
            {footerNav.map((group) => (
              <div key={group.title} className="flex flex-col gap-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cream/30">
                  {group.title}
                </p>
        ...
        {/* Alt çizgi */}
        <div className="mt-14 h-px w-full bg-gradient-to-r from-transparent via-cream/10 to-transparent" />
        ...
        <div className="mt-6 flex flex-col gap-3 text-[11px] text-cream/30 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            ...
            <a href="mailto:info@venti-ate.com" className="hover:text-cream/60 transition-colors">
              info@venti-ate.com
            </a>
          </div>
        </div>
