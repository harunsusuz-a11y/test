...
import { Palette, Save, Eye, EyeOff, GripVertical, Plus, Trash2, X } from "lucide-react";

type NavItem = { id: string; label: string; url: string; target: string; sort_order: number; is_active: boolean };
type ThemeSettings = {
...
const DEFAULT_THEME: ThemeSettings = {
  ...
  font_heading: "Obviously Wide", font_body: "Neue Haas Display",
...
const FONT_OPTIONS = ["Obviously Wide", "Neue Haas Display", "Inter", "Georgia", "Playfair Display", "Montserrat", "Lato", "Poppins"];
...
export default function TemaPage() {
  ...
  const T = (key: keyof ThemeSettings, value: unknown) =>
    setTheme(t => ({ ...t, [key]: value }));

  const load = useCallback(async () => {
    const { data: settings } = await supabase.from("settings").select("key,value").eq("setting_group", "theme");
    if (settings) {
    ...
    if (!menus) return;
    const hm = menus.find((m: { location: string }) => m.location === "header");
    const fm = menus.find((m: { location: string }) => m.location === "footer");
    if (hm) {
      const { data } = await supabase.from("menu_items").select("*").eq("menu_id", hm.id).is("parent_id", null).order("sort_order");
      setNavItems((data ?? []) as NavItem[]);
    }
    if (fm) {
      const { data } = await supabase.from("menu_items").select("*").eq("menu_id", fm.id).is("parent_id", null).order("sort_order");
      setFooterItems((data ?? []) as NavItem[]);
    }
  }, [supabase]);
  ...
  async function saveTheme() {
    setSaving(true);
    try {
      await Promise.all(Object.entries(theme).map(([key, value]) =>
        supabase.from("settings").upsert({ key, value: JSON.stringify(value), setting_group: "theme" }, { onConflict: "key" })
      ));
  ...
  async function addItem() {
    ...
    const { data: m } = await supabase.from("menus").select("id").eq("location", newItem.menu === "header" ? "header" : "footer").single();
    if (!m) { toastError("Menü bulunamadı"); return; }
    await supabase.from("menu_items").insert({ menu_id: m.id, label: newItem.label, url: newItem.url || "/", target: "_self", sort_order: 99, is_active: true });
    setNewItem(null);
    load();
  }
  ...
  function onDrop(e: React.DragEvent, targetId: string, items: NavItem[], setItems: (i: NavItem[]) => void, menu: string) {
    ...
    const ordered = next.map((item, idx) => ({ ...item, sort_order: idx }));
    setItems(ordered);
    setDragging(null); setDragOver(null);
    ordered.forEach((item, idx) => supabase.from("menu_items").update({ sort_order: idx }).eq("id", item.id));
  ...
  function NavEditor({ items, setItems, menu }: { items: NavItem[]; setItems: (i: NavItem[]) => void; menu: "header" | "footer" }) {
    return (
      <div>
        ...
        {newItem?.menu === menu && (
          <div style={{ background: "var(--adm-bg)", border: "1px solid rgba(200,162,107,0.5)", borderRadius: 8, padding: 14, marginBottom: 12, display: "flex", gap: 8, alignItems: "flex-end" }}>
            <div style={{ flex: 1 }}>
              <label style={sLabel}>Başlık</label>
              <input style={sInput} value={newItem.label} onChange={e => setNewItem(n => n ? { ...n, label: e.target.value } : null)} placeholder="Mağaza" />
            </div>
            <div style={{ flex: 1 }}>
              <label style={sLabel}>URL</label>
              <input style={sInput} value={newItem.url} onChange={e => setNewItem(n => n ? { ...n, url: e.target.value } : null)} placeholder="/magaza" />
            </div>
        ...
        {items.length === 0 ? (
          ...
          <div key={item.id} draggable
            ...
            style={{
              ...
              background: dragOver === item.id ? "rgba(200,162,107,0.1)" : "var(--adm-bg)",
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={saveTheme} disabled={saving}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, background: "#c8a26b", border: "none", color: "#000", cursor: saving ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 13, opacity: saving ? 0.7 : 1 }}>
      ...
      {preview && (
        <div style={{ marginBottom: 24, borderRadius: 10, overflow: "hidden", border: "1px solid var(--adm-border)" }}>
          ...
          <div style={{ background: theme.primary_color, color: theme.accent_color, padding: "16px 24px", fontSize: 12 }}>
        ...
        {TABS.map((t, i) => (
          <button key={i} onClick={() => setTab(i)}
            style={{
              ...
              border: tab === i ? "1px solid #c8a26b" : "1px solid var(--adm-border)",
              background: tab === i ? "rgba(200,162,107,0.12)" : "transparent",
              color: tab === i ? "#c8a26b" : "var(--adm-text-muted)",
      ...
      {tab === 0 && (
        <div style={sCard}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
            {([
              { key: "primary_color", label: "Ana Renk (Kahve)" },
              { key: "secondary_color", label: "İkinci Renk (Yeşil)" },
              { key: "accent_color", label: "Vurgu Rengi (Şeftali)" },
              { key: "bg_color", label: "Arkaplan" },
              ...
              <div key={key}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <input style={{ ...sInput, flex: 1 }} value={theme[key] as string} onChange={e => T(key, e.target.value)} placeholder="#000000" />
                </div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 20, borderRadius: 8, overflow: "hidden", display: "flex", height: 32 }}>
            {[theme.primary_color, theme.secondary_color, theme.accent_color, theme.bg_color].map((c, i) => (
      ...
      {tab === 1 && (
        <div style={sCard}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <label style={sLabel}>Başlık Fontu</label>
              <select style={sInput} value={theme.font_heading} onChange={e => T("font_heading", e.target.value)}>
                {FONT_OPTIONS.map(f => <option key={f} value={f}>{f}</option>)}
              </select>
              <p style={{ marginTop: 10, fontSize: 24, color: "var(--adm-text)", fontFamily: theme.font_heading }}>Fındığın Rafine Hali</p>
            </div>
            <div>
              <label style={sLabel}>Gövde Fontu</label>
              <select style={sInput} value={theme.font_body} onChange={e => T("font_body", e.target.value)}>
                {FONT_OPTIONS.map(f => <option key={f} value={f}>{f}</option>)}
              </select>
        ...
        <div>
          <div style={sCard}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <label style={sLabel}>Logo URL</label>
                <input style={sInput} value={theme.logo_url} onChange={e => T("logo_url", e.target.value)} placeholder="https://..." />
              </div>
              <div>
                <label style={sLabel}>Genişlik (px)</label>
                <input type="number" style={sInput} value={theme.logo_width} onChange={e => T("logo_width", parseInt(e.target.value))} />
              </div>
            </div>
            {theme.logo_url && (
              <div style={{ marginTop: 12, background: "var(--adm-bg)", borderRadius: 8, padding: 16, display: "inline-block" }}>
                <img src={theme.logo_url} alt="Logo" style={{ maxWidth: theme.logo_width, maxHeight: 60, objectFit: "contain" }} />
              </div>
            )}
          </div>
          <div style={sCard}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              <div>
                <label style={sLabel}>Duyuru Metni</label>
                <input style={sInput} value={theme.announcement_text} onChange={e => T("announcement_text", e.target.value)} />
              </div>
      ...
      {tab === 4 && (
        <div>
          <div style={sCard}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {([
                { key: "footer_tagline", label: "Slogan", placeholder: "Fındığın rafine hali." },
                { key: "footer_email", label: "İletişim E-postası", placeholder: "info@venti-ate.com" },
                { key: "footer_phone", label: "Telefon", placeholder: "+90 212 000 00 00" },
              ] as { key: keyof ThemeSettings; label: string; placeholder: string }[]).map(({ key, label, placeholder }) => (
                <div key={key}>
                  <label style={sLabel}>{label}</label>
                  <input style={sInput} value={theme[key] as string} onChange={e => T(key, e.target.value)} placeholder={placeholder} />
                </div>
              ))}
            </div>
          </div>
          <div style={sCard}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {([
                ...
                <div key={key}>
                  <label style={sLabel}>{label}</label>
                  <input style={sInput} value={theme[key] as string} onChange={e => T(key, e.target.value)} placeholder="https://..." />
                </div>
              ))}
            </div>
          </div>
