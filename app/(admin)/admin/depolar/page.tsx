...
interface Warehouse { id:string; name:string; address:string|null; is_active:boolean; is_default:boolean; created_at:string; }
const EMPTY = { name:"", address:"", is_active:true, is_default:false };
...
export default function AdminDepolar() {
  ...
  async function save() {
    ...
    setSaving(true);
    if (editId) await supabase.from("warehouses").update({ ...editing, address: editing.address||null }).eq("id", editId);
    else await supabase.from("warehouses").insert({ ...editing, address: editing.address||null });
          ...
          <div key={w.id} className="adm-card adm-card--hover">
            <div className="adm-card-header">
              <div>
                ...
                {w.is_default && <span className="adm-badge adm-badge--accent" style={{ marginLeft:8, fontSize:9 }}>Varsayılan</span>}
              </div>
              <span className={`adm-badge ${w.is_active?"adm-badge--green":"adm-badge--muted"}`}>{w.is_active?"Aktif":"Pasif"}</span>
            </div>
            <div className="adm-card-body">
              <div style={{ fontSize:12, color:"var(--adm-text-3)", marginBottom:12 }}>{w.address || "Adres girilmemiş"}</div>
              <button className="adm-btn adm-btn--ghost adm-btn--sm" onClick={() => { setEditing({ name:w.name, address:w.address||"", is_active:w.is_active, is_default:w.is_default }); setEditId(w.id); setOpen(true); }}>Düzenle</button>
            </div>
          </div>
        ))}
          ...
          <div className="adm-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="adm-modal-body">
              ...
              <div style={{ display:"flex", gap:16 }}>
                {[{k:"is_active",l:"Aktif"},{k:"is_default",l:"Varsayılan Depo"}].map(({k,l}) => (
