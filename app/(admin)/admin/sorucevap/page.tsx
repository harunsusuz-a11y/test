...
export default function SoruCevapPage() {
  ...
  const card: React.CSSProperties = { background:"var(--adm-surface)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:18, marginBottom:12 };
    ...
    <div style={{ padding:24 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <span style={{ fontSize:22, fontWeight:700, color:"var(--adm-text)" }}>Soru-Cevap Yönetimi</span>
        </div>
        <span style={{ fontSize:13, color:"var(--adm-text-muted)" }}>{questions.length} soru</span>
      </div>
        ...
        {(["all","pending","answered"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding:"6px 16px", borderRadius:6, cursor:"pointer", fontSize:13,
              ...
              color: filter===f ? "#c8a26b":"var(--adm-text-muted)" }}>
      ...
      {loading ? <p style={{ color:"var(--adm-text-muted)" }}>Yükleniyor…</p> :
        questions.length === 0 ? (
          <p style={{ color:"var(--adm-text-muted)", textAlign:"center", padding:"40px 0" }}>Soru bulunamadı.</p>
        ) : questions.map(q => (
          <div key={q.id} style={card}>
            ...
            <div style={{ fontSize:14, color:"var(--adm-text)", marginBottom:10, fontWeight:600 }}>❓ {q.question}</div>
            {q.answer && (
              <div style={{ fontSize:13, color:"var(--adm-text-muted)", background:"rgba(0,0,0,0.02)", padding:10, borderRadius:6, marginBottom:10 }}>
                💬 {q.answer}
              </div>
            )}
            ...
            {answering === q.id && (
              <div>
                <textarea
                  style={{ width:"100%", background:"var(--adm-surface)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:6, color:"var(--adm-text)", fontSize:13, padding:10, marginTop:8, resize:"vertical", boxSizing:"border-box" }}
                ...
                <div style={{ display:"flex", gap:8, marginTop:8 }}>
                  ...
                  <button style={{ ...btn, background:"rgba(0,0,0,0.03)", color:"var(--adm-text-muted)" }} onClick={() => { setAnswering(null); setAnswerText(""); }}>İptal</button>
                </div>
              </div>
            )}
            ...
            <div style={{ display:"flex", gap:8, marginTop:8 }}>
              ...
              <button style={{ ...btn, background:"rgba(0,0,0,0.03)", color:"var(--adm-text-muted)" }} onClick={() => togglePublish(q.id, q.status)}>
